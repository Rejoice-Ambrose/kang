window.onload = (function () {
  ...
  if (dropdownMenus.length !== 0) {
    const openDropDown = (element) =>
      element.classList.add("dropdown-menu--active");
  ...
  const headerMenuListItems = document.querySelectorAll(
    ".header__menu-list .menu-list__item"
  );
  ...
  headerMenuListItems.forEach((el) =>
  ...
  const arrowIconForWidgetHeader = document.querySelector(
    "#arrowIconForWidgetHeader"
  );
  ...
  setTimeout(() => {
    let widgetPriceBox = document.querySelectorAll(
    ...
    const arrowUrl = isGrown(widgetData.percent)
      ? "img/arrow-to-top.svg"
      : "img/arrow-to-down.svg";
    ...
    price.textContent = parseFloat(
      priceData["data"][9436]["quote"]["USD"]["price"]
    ).toFixed(9);
    priceHeader.textContent = parseFloat(
      priceData["data"][9436]["quote"]["USD"]["price"]
    ).toFixed(9);
    percent.textContent = widgetData.percent;

    let widgetInfoBox = document
      .querySelectorAll(".coinmarketcap-currency-widget > div > div")[1]
